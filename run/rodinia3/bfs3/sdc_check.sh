...
diff --side-by-side --suppress-common-lines result.txt ${APP_DIR}/result.txt > diff.log
if [ -s diff.log ]
then
  ACCEPT=1
  while read line
...
#    echo "OUTPUT_LINE[2,5]: ${OUTPUT_LINE[2]} and ${OUTPUT_LINE[5]}"
    if [[ ${OUTPUT_LINE[5]} == *"e"* ]]
    then
      output_line3=0 #`echo ${OUTPUT_LINE[3]} | sed -e 's/[eE]+*/\\*10\\^/'`
    else
      output_line3=${OUTPUT_LINE[5]}
    fi
    if [[ ${OUTPUT_LINE[2]} == 0.00 ]]
    then
      rate=`echo "scale=6;(($output_line3/0.1)*100)" | bc`
    else
      rate=`echo "scale=6; (((${OUTPUT_LINE[2]}-$output_line3)/${OUTPUT_LINE[2]})*100)" | bc`
    fi
...
#################################
## FIXME: Sometimes stdout is different from golden_stdout but SDC happens only
##        diff.log is non-zero
##################################
