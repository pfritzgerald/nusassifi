...
class BenchImpl(object):
    ...
    def makefile(self, benchmark, target=None, action=None, sassi_option=None, platform=None, opt={}):
        """Run this implementation's makefile."""
        
        self.platform = platform
        self. sassi_option = sassi_option
        ...
        def perform():
            ...
            if self.sassi_option == None: sassi_option = 'none'
            else: sassi_option = self.sassi_option

            env={'SRCDIR':srcdir,
                 'BUILDDIR':builddir + '_' + platform,
                 'BIN':path.join(builddir+'_'+platform,benchmark.name),
                 'PARBOIL_ROOT':globals.root,
                 'SASSI_OPTION':sassi_option,
                 'PLATFORM':platform,
                 'BUILD':self.name}
            env.update(opt)
    ...
    def build(self, benchmark, sassi_option, platform):
        """Build an executable of this benchmark implementation."""
        return self.makefile(benchmark, action='build', sassi_option=sassi_option, platform=platform)
