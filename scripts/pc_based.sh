...
#
################################################
# Step 5: Build the app for error injectors
################################################
printf "\n-------------\nStep 5: Prepare application for error injection\n"
make OPTION=pc_inst_injector
...
#cd -
cd $SASSIFI_HOME/scripts/
python generate_injection_list.py $inst_rf pc
if [ $? -ne 0 ]; then
    echo "Return code was not zero: $?"
		exit -1;
fi
...
################################################
printf "\n------------\nStep 8: Run the error injection campaign\n"
python run_injections.py $inst_rf $where_to_run pc
